...
public class GameManager : MonoBehaviour
{
    ...
    private void Start ()
    {
        ...
        GoldInfo.text = $"Gold:{ UserDataManager.Progress.Gold.ToString("F0") }";
    }
    ...
    private void AddAllResources ()
    {
        ...
        Debug.Log("config count : " + ResourcesConfigs.Length);
    ...
    private void CollectPerSecond ()
    {
        ...
        foreach (ResourceController resource in _activeResources)
        {
            if (resource.IsUnlocked)
            {
                Debug.Log("adding gold : " + resource.GetOutput());
    ...
    public void AddGold (double value)
    {
        ...
        {
            Debug.Log("saved to local");
        }

        if(_saveDelayCounter < 0)
        {
